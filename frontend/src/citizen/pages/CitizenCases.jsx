...
export default function CitizenCases() {
  ...
  const handleOpenCase = useCallback(
    (caseItem) => {
      ...
      if (!id) return;
      navigate(`/citizen/cases/${safeStatus}/${id}`, {
        state: {
          caseId: id,
          // For citizens, highlight the assigned lawyer in the detail view
          citizenName: caseItem?.lawyer?.name || "Lawyer",
          citizenEmail: caseItem?.lawyer?.email,
    ...
    return (
      <div className="space-y-4">
        ...
        <EmptyState
          ...
          message="Cases will appear here once created from accepted consultations."
        />
      </div>
    );
