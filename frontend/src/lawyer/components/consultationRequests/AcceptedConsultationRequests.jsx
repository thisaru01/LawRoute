...
export default function AcceptedConsultationRequests({
  requests,
  isLoading,
  error,
  onRetry,
  ...
  if (!Array.isArray(requests) || requests.length === 0) {
    return (
      <p className="mt-6 text-sm text-muted-foreground">
        No accepted consultation requests.
      </p>
    );
  }
  ...
  return (
    <div className="mt-6 space-y-4">
      {requests.map((request) => {
        ...
        return (
          <Card key={request?._id || `${citizenName}-${createdAt}`}>
            <CardHeader>
              <CardTitle>Request from {citizenName}</CardTitle>
              <CardDescription>
